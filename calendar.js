...
const DEFAULT_COLOR = '#38f100'; // team green, used when an event has no color set

export default async function handler(req, res) {
  try {
    const API_KEY = process.env.GOOGLE_CALENDAR_API_KEY;
    const CAL_ID = process.env.GOOGLE_CALENDAR_ID || 'nextlevelbmx239@gmail.com';
    if (!API_KEY) {
    ...
    const events = (data.items || [])
      ...
      .map(e => ({
        ...
        htmlLink: e.htmlLink || '',
        color: COLOR_MAP[e.colorId] || DEFAULT_COLOR,
      }));
