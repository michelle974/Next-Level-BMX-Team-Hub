  ...
  try {
    switch (action) {
      ...
      case 'roster': {
        ...
        const idsFrom = s => String(s || '').split(',').map(x => x.trim()).filter(Boolean);

        const contactOf = (f, forceShow) => ({
          phone: (forceShow || (f.PHONE_PUBLIC || '').toLowerCase() === 'true') ? f.PHONE || '' : '',
          email: (forceShow || (f.EMAIL_PUBLIC || '').toLowerCase() === 'true') ? f.EMAIL || '' : '',
        });

        const list = riders.map(r => {
          const f = r.fields;
          const c = contactOf(f, isAdmin);
          const linkedParentGids = idsFrom(f.LINKED_PARENTS);

          const kids = parents
            .filter(p => linkedParentGids.includes(p.gid) ||
                         idsFrom(p.fields.LINKED_RIDERS).includes(r.gid))
            .map(p => {
              const pc = contactOf(p.fields, isAdmin);
              return { gid: p.gid, name: p.name, phone: pc.phone, email: pc.email };
            });
        ...
        if (isAdmin) {
          unlinked = parents
            .filter(p => {
              const hasRiders = idsFrom(p.fields.LINKED_RIDERS).length > 0;
              const claimedBy = riders.some(r =>
                idsFrom(r.fields.LINKED_PARENTS).includes(p.gid));
              return !hasRiders && !claimedBy;
            })
            .map(p => ({
              gid: p.gid,
              name: p.name,
              ...
              role: p.fields.ROLE || '',
              pendingMatch: p.fields.PENDING_MATCH || '',
            }));
        }
      ...
      case 'adminLink': {
        ...
        const pf = parseNotes(parent.notes || '');
        const rf = parseNotes(rider.notes || '');

        const split = s => String(s || '').split(',').map(x => x.trim()).filter(Boolean);
        let pRiders = split(pf.LINKED_RIDERS);
        let rParents = split(rf.LINKED_PARENTS);

        if (unlink) {
          pRiders = pRiders.filter(x => x !== riderGid);
          rParents = rParents.filter(x => x !== parentGid);
        } else {
          if (!pRiders.includes(riderGid)) pRiders.push(riderGid);
          if (!rParents.includes(parentGid)) rParents.push(parentGid);
        }

        await patchTask(parentGid, {
          LINKED_RIDERS: pRiders.join(', '),
          PENDING_MATCH: '',
        });
        await patchTask(riderGid, { LINKED_PARENTS: rParents.join(', ') });
      ...
      case 'myRiders': {
        ...
        const split = s => String(s || '').split(',').map(x => x.trim()).filter(Boolean);
        const mine = split(f.LINKED_RIDERS);

        const out = riders
          .filter(r => mine.includes(r.gid) ||
                       split(r.fields.LINKED_PARENTS).includes(gid))
          .map(r => ({
            gid: r.gid,
            name: r.name,
        ...
        // "Both" and adult riders also sign for themselves
        if (role === 'RIDER' || role === 'BOTH') {
          out.unshift({
            gid,
            name: me.name + ' (you)',
      ...
      case 'submitForm': {
        ...
        if (form.name && form.name.trim()) {
          await asana(`/tasks/${target}`, {
            method: 'PUT',
            body: JSON.stringify({ data: { name: form.name.trim() } }),
          });
        }
