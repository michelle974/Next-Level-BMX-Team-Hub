...
const cleanNum = v => String(v ?? '').replace(/[^0-9]/g, '').slice(0, 3);
  ...
  try {
    switch (action) {
        ...
        return res.json({
          ...
          config: {
            ...
            jersey: {
              ...
              nameMax: parseInt(cfg.JERSEY_NAME_MAX || '12', 10),
      ...
      case 'submitJersey': {
        ...
        const nameMax = parseInt(cfg.JERSEY_NAME_MAX || '12', 10);
        ...
        for (const e of extras) {
          ...
          const qty = Math.min(99, Math.max(1, parseInt(e.qty || '1', 10) || 1));
          if (!size && !number && !backName) continue;
          ...
          cleanExtras.push({ size, number, name: backName, qty });
        }

        const riderQty = riders.filter(r => r && r.gid && cleanVal(r.size, 8)).length;
      ...
      case 'adminSetJersey': {
        ...
        await patchTask(riderGid, clear ? {
          ...
          JERSEY_STATUS: undefined, JERSEY_DATE: undefined,
        } : {
      ...
      case 'adminJerseyOrders': {
        ...
        for (const h of fam.values()) {
          ...
          const riderQty = h.riders.filter(r => r.done).length;
        ...
        for (const f of families) {
          for (const r of f.riders) if (r.done) sizeTotals[r.size] = (sizeTotals[r.size] || 0) + 1;
